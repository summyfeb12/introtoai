...
/**
 * This handler takes a log message sent to the JDK's logging system and sends
 * it to log4j.
 * 
...
public class JDKLogHandler extends java.util.logging.Handler {
    /**
     * Removes any handlers attached to the JDK root logger, and adds an
     * instance of this.
     */
    ...
    public void publish(final java.util.logging.LogRecord record) {
        ...
        } else if (jdkLevel == java.util.logging.Level.INFO
                || jdkLevel == java.util.logging.Level.CONFIG) {
        ...
        } else if (jdkLevel == java.util.logging.Level.FINER
                || jdkLevel == java.util.logging.Level.FINEST) {
            log4jLevel = org.apache.log4j.Level.TRACE;
        } else {
            assert false : String.format("Unknown JDK logging level %s.",
                    jdkLevel);
            return;
        }
        ...
        org.apache.log4j.Logger log4jLogger = org.apache.log4j.Logger
                .getLogger(loggerName);

        org.apache.log4j.spi.LoggingEvent event = new org.apache.log4j.spi.LoggingEvent(
                record.getSourceClassName(), log4jLogger, record.getMillis(),
                log4jLevel, record.getMessage(), record.getThrown());
        // event.getLocationInformation();

        // Call the logger's appenders on a new event created based on the JDK
        // log record.
