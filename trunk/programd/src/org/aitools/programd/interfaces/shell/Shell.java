...
public class Shell extends Thread {
    ...
    public Shell(final InputStream in, final PrintStream out,
            final PrintStream err, final PrintStream prompt) {
     ...
     * @param coreToUse
     */
    public void attachTo(final Core coreToUse) {
        this.core = coreToUse;
        ...
        this.clientNamePredicate = this.core.getSettings()
        ...
        Element shellCommandSet = XMLKit.getFirstElementNamed(this.core
                .getPluginConfig().getDocumentElement(), "shell-commands");
        if (shellCommandSet != null) {
            List<Element> commands = XMLKit.getAllElementsNamed(
                    shellCommandSet, "command");
            if (commands != null) {
                for (Element commandElement : commands) {
                    String classname = commandElement.getAttribute("class");
                    List<Element> parameterElements = XMLKit
                            .getAllElementsNamed(commandElement, "parameter");
    ...
    public void run() {
        if (this.core == null) {
            throw new DeveloperError(
                    "Must attach the shell to a Core before calling run()!",
                    new NullPointerException());
        }

        showMessage("Interactive shell: type \"" + EXIT + "\" to shut down; \""
                + HelpCommand.COMMAND_STRING + "\" for help.");
        Bot bot = this.bots.getABot();
        if (bot == null) {
            throw new UserError("No bot to talk to!",
                    new NullPointerException());
        }
        ...
        while (true /* && this.core.getStatus() == Core.Status.READY */) {
            ...
            if (commandLine.indexOf('#') == 0) {
                ...
                showConsole(this.botName, XMLKit.filterViaHTMLTags(this.core
                        .getResponse(commandLine, this.hostname, this.botid)));
            }
    ...
    private void noShell() {
        ...
        while (true) {
            try {
                ...
                this.core.getLogger().warn(
                        "Shell was interrupted; shell will not run anymore.");
            }
        }
    }
     ...
     * Prints the command line to the console so it's possible to see what was
     * attempted.
     * 
     * @param commandLine
     *            the command line to process
     * @throws NoSuchCommandException
     *             if the command line did not contain a command that could be
     *             processed
     */
    public void processCommandLine(final String commandLine)
            throws NoSuchCommandException {
        this.consolePrompt.println(this.hostname + PROMPT + commandLine);
        this.commandRegistry.getHandlerFor(commandLine).handle(commandLine,
                this);
    }
    ...
    private void showPrompt() {
        if (this.getState() != Thread.State.NEW) {
            promptConsole('['
                    + this.botName
                    + "] "
                    + this.predicateMaster.get(this.clientNamePredicate,
                            this.hostname, this.botid).trim());
        }
    }
     ...
     */
    private void promptConsole(final String preprompt) {
        if (this.midLine) {
     ...
     */
    private void showConsole(final String preprompt, final String[] message) {
     ...
     */
    public void printlnOut(final String message) {
        if (this.midLine) {
     ...
     *            the message to print
     */
    public void printlnErr(final String message) {
        if (this.midLine) {
     ...
     * @param newBotID
     */
    public void switchToBot(final String newBotID) {
        if (!this.bots.include(newBotID)) {
            showError("That bot id is not known. Check your startup files.");
            return;
        }
        this.botid = newBotID;
        this.botName = this.bots.getBot(newBotID).getPropertyValue(
                this.botNamePredicate);
        showMessage("Switched to bot \"" + newBotID + "\" (name: \""
                + this.botName + "\").");
        // Send the connect string and print the first response.
        showConsole(this.botName, XMLKit.filterViaHTMLTags(this.core
                .getResponse(this.core.getSettings().getConnectString(),
     ...
     */
    private void commandCommandable(final String command)
