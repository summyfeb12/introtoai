...
public class Core {
    ...
    /**
     * Initializes a new Core object with default property values and the given
     * base URL.
     * 
     * @param base
     *            the base URL to use
     */
    public Core(final URL base) {
    ...
    /**
     * Initializes a new Core object with the properties from the given file and
     * the given base URL.
     * 
     * @param base
     *            the base URL to use
     ...
     */
    public Core(final URL base, final URL propertiesPath) {
        this.baseURL = base;
    ...
    /**
     * Initializes a new Core object with the given CoreSettings object and the
     * given base URL.
     * 
     * @param base
     *            the base URL to use
     ...
     *            the settings to use
     */
    public Core(final URL base, final CoreSettings settingsToUse) {
    ...
    protected void start() {
        ...
        // Use the stdout and stderr appenders in a special way, if they are
        // defined.
        ConsoleStreamAppender stdOutAppender = ((ConsoleStreamAppender) Logger
                .getLogger("programd").getAppender("stdout"));
        ...
        ConsoleStreamAppender stdErrAppender = ((ConsoleStreamAppender) Logger
                .getLogger("programd").getAppender("stderr"));
        ...
        // Set up an interception of calls to the JDK logging system and
        // re-route to log4j.
        ...
        this.parser = XMLKit.getSAXParser(URLTools.contextualize(
                FileManager.getWorkingDirectory(), AIML_SCHEMA_LOCATION),
                "AIML");
        ...
        this.multiplexor = ClassUtils.getSubclassInstance(Multiplexor.class,
                this.settings.getMultiplexorImplementation(), "Multiplexor",
                this);
        ...
        try {
            this.pluginConfig = XMLKit.getDocumentBuilder(
                    URLTools.contextualize(FileManager.getWorkingDirectory(),
                            PLUGINS_SCHEMA_LOCATION), "plugin configuration")
                    .parse(URLTools.contextualize(this.baseURL,
                            this.settings.getConfLocationPlugins()).toString());
        } catch (IOException e) {
            this.logger.error("IO error trying to read plugin configuration.",
                    e);
        ...
        try {
            ...
            // Start loading bots.
            loadBots(URLTools.contextualize(this.baseURL,
                    this.settings.getStartupFilePath()));
    ...
    private void setupInterpreter() throws UserError, DeveloperError {
        if (this.settings.javascriptAllowed()) {
            ...
            try {
                this.interpreter = (Interpreter) Class.forName(
                        javascriptInterpreterClassname).newInstance();
            } catch (Exception e) {
                throw new DeveloperError(
                        "Error while creating new instance of JavaScript interpreter.",
                        e);
            }
        } else {
     ...
     * @param botid
     */
    public void load(final URL path, final String botid) {
        ...
        if (path.getProtocol().equals(FileManager.FILE)) {
            ...
            if (spec.indexOf(ASTERISK) != -1) {
                ...
                if (files != null) {
                    for (File file : files) {
                        load(URLTools.contextualize(URLTools.getParent(path),
                                file.getAbsolutePath()), botid);
                    }
                }
                return;
            }
        ...
        if (this.graphmaster.hasAlreadyLoaded(path)) {
            ...
            if (this.logger.isDebugEnabled()) {
                this.logger
                        .debug(String
                                .format("Graphaster has already loaded \"%s\" for some other bot.",
                                        path));
            }
    ...
    /**
     * Reloads a file&mdash;it is not necessary to specify a particular botid
     * here, because a reload of a file for one botid suffices for all bots
     * associated with that file.
     * 
     * @param path
     * @throws IllegalArgumentException
     *             if the given path is not actually loaded for <em>any</em> bot
     */
    public void reload(final URL path) {
        Set<String> botids = this.graphmaster.getURLCatalog().get(path);
        if (botids == null || botids.size() == 0) {
            throw new IllegalArgumentException(
                    "Called Core.reload() with a path that is not loaded by any bot.");
        }
     ...
     * @param botid
     */
    private void doLoad(final URL path, final String botid) {
        try {
            AIMLReader reader = new AIMLReader(this.graphmaster, path, botid,
                    this.bots.getBot(botid), this.settings
            ...
            try {
                ...
                this.logger
                        .warn("The XML reader in use does not support lexical handlers -- CDATA will not be handled.",
                                e);
            } catch (SAXNotSupportedException e) {
                this.logger
                        .warn("The XML reader in use cannot enable the lexical handler feature -- CDATA will not be handled.",
                                e);
            } catch (SAXException e) {
                this.logger
                        .warn("An exception occurred when trying to enable the lexical handler feature on the XML reader -- CDATA will not be handled.",
                                e);
            }
            ...
            this.logger.warn(
                    String.format("Error reading \"%s\": %s",
                            URLTools.unescape(path), e.getMessage()), e);
        } catch (SAXException e) {
            this.logger.warn(
                    String.format("Error parsing \"%s\": %s",
                            URLTools.unescape(path), e.getMessage()), e);
        }
    }
     ...
     *            the path to check
     * @param bot
     *            the bot for whom to check
     * @return whether or not the given path should be loaded
     */
    private boolean shouldLoad(final URL path, final Bot bot) {
        if (bot == null) {
    ...
    /**
     * Sets "loadtime" mode (so accidentally duplicated paths in a load config
     ...
     *            the input to send
     */
    public synchronized void processResponse(final String input) {
        ...
        // throw new
        // DeveloperError("Check that the Core is ready before sending it messages.",
        // new CoreNotReadyException());
    }
     ...
     * @return the response
     */
    public synchronized String getResponse(final String input,
            final String userid, final String botid) {
        if (this.status == Status.READY) {
            return this.multiplexor.getResponse(input, userid, botid);
        }
        // otherwise...
        // throw new
        // DeveloperError("Check that the Core is running before sending it messages.",
        // new CoreNotReadyException());
        return null;
    }
    ...
    /**
     * Notes the given Throwable and advises that the Core may no longer be
     * stable.
     * 
     * @param e
     *            the Throwable to log
     */
    public void alert(final Throwable e) {
     ...
     * 
     * @param t
     *            the thread in which the Throwable was thrown
     * @param e
     *            the Throwable to log
     */
    public void alert(final Thread t, final Throwable e) {
     ...
     * 
     * @param description
     *            the description of the Throwable
     * @param e
     *            the Throwable to log
     */
    public void alert(final String description, final Throwable e) {
     ...
     *            the Throwable to log
     */
    public void alert(final String description, final Thread t,
            final Throwable e) {
    ...
    class UncaughtExceptionHandler implements Thread.UncaughtExceptionHandler {
        ...
        public void uncaughtException(final Thread t, final Throwable e) {
            System.err.println("Uncaught exception "
                    + e.getClass().getSimpleName() + " in thread \""
                    + t.getName() + "\".");
            ...
            System.err
                    .println("Core has crashed.  Shutdown may not have completed properly.");
        }
    }
     ...
     */
    public void loadBots(final URL path) {
        if (this.settings.useWatcher()) {
        ...
        try {
            ...
            this.logger.error(
                    "Processor exception during startup: "
                            + e.getExplanatoryMessage(), e);
        }
        if (path.getProtocol().equals(FileManager.FILE)) {
            FileManager.popWorkingDirectory();
        }
    ...
    /**
     * Loads a bot from the given path. Will only work right if the file at the
     * path actually has a &gt;bot&lt; element as its root.
     * 
     * @param path
     *            the bot config file
     * @return the id of the bot loaded
     */
    public String loadBot(final URL path) {
     ...
     *            the bot to unload
     */
    public void unloadBot(final String id) {
        if (!this.bots.include(id)) {
            this.logger
                    .warn("Bot \"" + id + "\" is not loaded; cannot unload.");
            return;
        }
    ...
    public Bots getBots() {
        ...
        throw new NullPointerException(
                "The Core's Bots object has not yet been initialized!");
    }

    /**
     * @param id
     *            the id of the bot desired
     * @return the requested bot
     */
    public Bot getBot(final String id) {
